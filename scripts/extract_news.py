...
def extract_markdown_from_html(filepath: Path) -> tuple[str, str, str]:
    ...
    # --- 提取各板块 ---
    sections = re.split(r'<div class="section">', html)
    
    for section in sections[1:]:  # 跳过第一个（header之前的部分）
        # 提取 section 标题
        ...
        # 提取每个 card
        cards = re.split(r'<div class="card">', section)
        for ci, card in enumerate(cards[1:], 1):  # 跳过分割产生的空段
            # 提取标签
            ...
            # 提取 why-matters
            why_match = re.search(r'<div class="(?:why-matters|card-why)">(.*?)</div>', card, re.DOTALL)
            why = ""
            if why_match:
            ...
            # 组装 card 的 Markdown
            md_lines.append(f"### {ci}. {title}")
            md_lines.append("")
            if tag:
                md_lines.append(f"> 标签: {tag}")
                md_lines.append("")
            if body:
                md_lines.append(body)
                md_lines.append("")
            if why:
                md_lines.append(f"💡 **值得关注**: {why}")
                md_lines.append("")
...
def generate_post_ts(date_str: str, post_id: str, title: str, content: str) -> str:
    ...
    summary = plain_content[:100] + "..." if len(plain_content) > 100 else plain_content
